...
package com.yoidles.android.bronze.renderer.util;
...
import com.yoidles.android.bronze.renderer.LabelObjectManager;
import com.yoidles.android.bronze.units.GeocentricCoordinates;
import com.yoidles.android.bronze.util.MathUtil;
import com.yoidles.android.bronze.util.VectorUtil;
...
public class SkyRegionMap<RegionRenderingData> {
  ...
  */
   ...
   */
  public static int getObjectRegion(GeocentricCoordinates position, LabelObjectManager.Label l) {
	    return getObjectRegionData(position, l).region;
	  }

// TODO RLP Clumsy code thrown in late at night to debug; fix it before sharing
  ...
  public static ObjectRegionData getObjectRegionData(GeocentricCoordinates position, LabelObjectManager.Label l) {
    // The closest region will minimize the angle between the vectors, which
    // will maximize the dot product, so we just return the region which
    // does that.
    ObjectRegionData data = new ObjectRegionData();
    ...
    if (data.regionCenterDotProduct < MathUtil.cos(REGION_COVERAGE_ANGLE_IN_RADIANS)) {
      Log.e("ActiveSkyRegionData",
            "Object put in region, but outside of coverage angle." +
            "Angle was " + MathUtil.acos(data.regionCenterDotProduct) + " vs "  +
            REGION_COVERAGE_ANGLE_IN_RADIANS + ". Region was " + data.region + ", Label was " + l.getText());
    }
