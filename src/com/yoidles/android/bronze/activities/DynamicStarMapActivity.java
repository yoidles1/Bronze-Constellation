...
import com.yoidles.android.bronze.activities.util.ActivityLightLevelChanger.NightModeable;
...
import com.yoidles.android.bronze.control.AstronomerModel.Pointing;
import com.yoidles.android.bronze.control.AstronomerModelImpl;
import com.yoidles.android.bronze.control.ControllerGroup;
import com.yoidles.android.bronze.control.MagneticDeclinationCalculatorSwitcher;
import com.yoidles.android.bronze.kml.KmlManager;
import com.yoidles.android.bronze.layers.LayerManager;
import com.yoidles.android.bronze.renderer.RendererController;
import com.yoidles.android.bronze.renderer.SkyRenderer;
import com.yoidles.android.bronze.renderer.util.AbstractUpdateClosure;
import com.yoidles.android.bronze.search.SearchResult;
import com.yoidles.android.bronze.touch.DragRotateZoomGestureDetector;
import com.yoidles.android.bronze.touch.GestureInterpreter;
import com.yoidles.android.bronze.touch.MapMover;
...
import com.yoidles.android.bronze.util.MathUtil;
import com.yoidles.android.bronze.util.MiscUtil;
import com.yoidles.android.bronze.util.OsVersions;
import com.yoidles.android.bronze.views.ButtonLayerView;
import com.yoidles.android.bronze.views.WidgetFader;
import com.yoidles.android.bronze.views.WidgetFader.Fadeable;
...
public class DynamicStarMapActivity extends Activity implements OnSharedPreferenceChangeListener {
  ...
  public void onCreate(Bundle icicle) {
    ...
    model = BronzeConstellationApplication.getModel();
    ...
    Log.d("RLP","in DSMA, geocoder =" + geocoder2 + ", model = " + model);
    
	((AstronomerModelImpl)model).setGeocoder(geocoder2);
    
    layerManager = BronzeConstellationApplication.getLayerManager(getAssets(),
                                                        sharedPreferences,
                                                        getResources(),
                                                        this);
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    switch (item.getItemId()) {
      ...
      case R.id.menu_item_time:
          ...
          break;
      case R.id.menu_item_earthmarks:
          Log.d(TAG, "Starting Earthmarks Dialog from menu");
          Analytics.getInstance(this).trackEvent(Analytics.USER_ACTION_CATEGORY,
              Analytics.MENU_ITEM, Analytics.EARTHMARKS_OPENED_LABEL, 1);
// TODO RLP figure out if we need this...
//          if (!timePlayerUI.isShown()) {
//            Log.d(TAG, "Just showing the user the current earthmarks");
//            controller.goTimeTravel(new Date());
//          } else {
//            Log.d(TAG, "Resuming current time travel dialog.");
//          }
          showDialog(DialogFactory.DIALOG_ID_EARTHMARKS);
          break;
  ...
  void activateSearchTarget(GeocentricCoordinates target, final String searchTerm) {
    Log.d(TAG, "RLP Item " + searchTerm + " selected");
    // Store these for later.
    searchTarget = target;
    searchTargetName = searchTerm;
    Log.d(TAG, "RLP Searching for target=" + target);
  ...
  public AstronomerModelImpl getModel() {
    return model;
  }
